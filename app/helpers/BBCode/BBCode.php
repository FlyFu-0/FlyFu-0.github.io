...
class BBCode
{
		...
		switch ($tag->name) {
			...
			case self::TAG_NAME_LI_STAR:
				if ($tag->opening) {
					 ...
					 */
					if (isset($openTags['list']) and sizeof($openTags['list'])
						> 0
					) {
						$tag->opening = false;
			...
			case self::TAG_NAME_QUOTE:
				if ($tag->opening) {
					if ($tag->property) {
						$code = '<blockquote><span class="author">'
							. $tag->property . ':</span><br/>';
					} else {
						$code = '<blockquote>';
					}
				} else {
			...
			case self::TAG_NAME_YOUTUBE:
				if ($tag->opening) {
					$code
						= '<iframe class="youtube-player" type="text/html" width="'
						. $this->youTubeWidth . '"\
	...
	{
		return ($needle === '' or mb_substr($haystack, -mb_strlen($needle))
			=== $needle);
	}
